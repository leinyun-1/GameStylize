...
class SDVideoPipelineRunner:
    ...
    def run(self, config):
        ...
        unet_state_dict = load_file('/root/leinyu/model/stable-diffusion-v1-5/unet/diffusion_pytorch_model.safetensors')
        unet_state_dict = SDUNetStateDictConverter().from_diffusers(unet_state_dict)
        pipe.unet.load_state_dict(unet_state_dict,strict=True)
        # from diffsynth.models.lora import GeneralLoRAFromPeft
        # lora_path = '/mnt/aigc_cq/private/leinyu/code/GameVideoControl/exp_output/0715_sd15_lora_csgo/lightning_lora_ckpts/lora-epoch=03-step=002000.ckpt'
        # lora_state_dict = torch.load(lora_path,map_location="cpu")
        # lora_state_dict = SDUNetStateDictConverter().from_diffusers(lora_state_dict)
        # GeneralLoRAFromPeft().load(pipe.unet,lora_state_dict)
