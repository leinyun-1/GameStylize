...
from ..schedulers.flow_match_unipc import FlowUniPCMultistepScheduler 
...
class WanVideoPipeline(BasePipeline):
    ...
    def prepare_vace_kwargs(
        ...
        if vace_video is not None or vace_mask is not None or vace_reference_image is not None:
            ...
            vace_mask_latents = torch.nn.functional.interpolate(vace_mask_latents, size=((vace_mask_latents.shape[2] + 3) // 4, vace_mask_latents.shape[3], vace_mask_latents.shape[4]), mode='nearest-exact') # mask不需要encode，只需要下采样
            ...
            if vace_reference_image is None:
                ...
                vace_reference_latents = torch.concat((vace_reference_latents, torch.zeros_like(vace_reference_latents)), dim=1) # 为了在通道上相同
                vace_video_latents = torch.concat((vace_reference_latents, vace_video_latents), dim=2) # b 2c t+1 h w 
                vace_mask_latents = torch.concat((torch.zeros_like(vace_mask_latents[:, :, :1]), vace_mask_latents), dim=2) # b 1 t+1 h w 
                ...
                latents = torch.concat((noise, latents), dim=2)  # b c t+1 h w 
    ...
    def __call__(
        ...
        self.scheduler.set_timesteps(num_inference_steps, device=self.device, shift=sigma_shift)
        ...
        if isinstance(prompt, str):
            ...
            prompt['context'] = prompt['context'].to(device=self.device, dtype=self.torch_dtype)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            timestep = torch.stack([timestep] * latents.shape[0], dim=0).to(dtype=self.torch_dtype, device=self.device)  # (b,)

            # Inference
            noise_pred_posi = model_fn_wan_video(
                self.dit, motion_controller=self.motion_controller, vace=self.vace,
                x=latents, timestep=timestep, action=action,
                **prompt_emb_posi, **image_emb, **extra_input,
                **tea_cache_posi, **usp_kwargs, **motion_kwargs, **vace_kwargs,sliding_window_size=sliding_window_size, sliding_window_stride=sliding_window_stride,
            )
            if cfg_scale != 1.0:
                noise_pred_nega = model_fn_wan_video(
                    ...
                    **tea_cache_nega, **usp_kwargs, **motion_kwargs, **vace_kwargs,sliding_window_size=sliding_window_size, sliding_window_stride=sliding_window_stride,
                )
...
class WanVideoPipelineNonunipc(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        if device is None: device = model_manager.device
        if torch_dtype is None: torch_dtype = model_manager.torch_dtype
        pipe = WanVideoPipelineNonunipc(device=device, torch_dtype=torch_dtype)
        pipe.fetch_models(model_manager)
        if use_usp:
            from xfuser.core.distributed import get_sequence_parallel_world_size
            from ..distributed.xdit_context_parallel import usp_attn_forward, usp_dit_forward
...
def model_fn_wan_video( # 为什么要重新实现core forward，就是因为旁路分支要注入脊椎！ 而这些post train 都没有影响主block参数。这是一种解耦设计范式。
    dit: WanModel,
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        ...
        return TemporalTiler_BCTHW().run(
            ...
            tensor_names=["x", "y", "vace_context"],  # vace_context 也有时间t维度
            batch_size=1
        )
    ...
    if vace_context is not None:
        vace_hints = vace(x, vace_context, context, t_mod, freqs) # 按理来说x不是旁路输入，vace的x'在一开始会和x相加
...
def model_fn_wan_video_df( # 为什么要重新实现core forward，就是因为旁路分支要注入脊椎！ 而这些post train 都没有影响主block参数。这是一种解耦设计范式。
    dit: WanModel,
    motion_controller: WanMotionControllerModel = None,
    vace: VaceWanModel = None,
    x: torch.Tensor = None,
    ...
    **kwargs,
    ...
    if timestep.dim() == 2:
        ...
        df = 0
    t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, timestep.flatten()))
    t_mod = dit.time_projection(t).unflatten(1, (6, dit.dim))
    if motion_bucket_id is not None and motion_controller is not None:
        t_mod = t_mod + motion_controller(motion_bucket_id).unflatten(1, (6, dit.dim))
    context = dit.text_embedding(context)
    ...
    if df == 1:
        t_mod = t_mod.view(b, f, 1, 1, 6, dit.dim)
        vace_t_mod = t_mod[:,-1,0,0,:,:] # b 6 c  只针对[0,,0,t,,,t] 这种情况
        t_mod = t_mod.repeat(1, 1, h, w, 1, 1).flatten(1, 3) # b （f*h*w） 6 c 
        t_mod = t_mod.transpose(1, 2).contiguous() # b 6 （f*h*w） c
        
        t = t.view(b, f, 1, 1, dit.dim)
        t = t.repeat(1, 1, h, w, 1).flatten(1, 3) # b （f*h*w） c
    ...
    if vace_context is not None:
        vace_hints = vace(x, vace_context, context, vace_t_mod, freqs) # 按理来说x不是旁路输入，vace的x'在一开始会和x相加
