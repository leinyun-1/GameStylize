...
class LightningModelForTrain(pl.LightningModule):
    def __init__(self,args):
        super().__init__()
        dit_path='/dockerdata/VACE-Wan2.1-1.3B-Preview/diffusion_pytorch_model.safetensors'
        ...
        if train_architecture == "lora":
            ...
            if self.pipe.vace is not None and self.args.train_vace_lora==True:
                self.add_lora_to_model(
                    self.pipe.vace,
                    lora_rank=lora_rank,
                    lora_alpha=lora_alpha,
    ...
    def training_step(self, batch, batch_idx):
        ...
        vace_context = vace_context[:,:,t:t+self.args.latent_len]
        f,h,w = latents.shape[2:]
        vace_kwargs = {"vace_context": vace_context, "vace_scale": 1.0}
        ...
        # Compute loss
        noise_pred = model_fn_wan_video(self.pipe.denoising_model(),vace=self.pipe.vace,x=noisy_latents, timestep=timestep, **prompt_emb,**vace_kwargs)
    ...
    def configure_optimizers(self):
        trainable_modules = filter(lambda p: p.requires_grad, list(self.pipe.denoising_model().parameters()) + list(self.pipe.vace.parameters()))
...
if __name__ == "__main__":
    ...
    config = {
    "name": '0825_wan_lora_vace_delta',
    'train_architecture': 'lora',
    'train_vace_lora': False,
    'diff_forcing': False,
    'history_guide': False,
    'history_guide_len': 1,
    'learning_rate': 1e-4,
    'dataset': 'delta',
    'dataset_root': '/root/leinyu/data/fps/latent_d2v/delta',    
    'latent_len': 21,
    'devices': [6,7],
    'lora_rank': 32,
    'batch_size': 2,
